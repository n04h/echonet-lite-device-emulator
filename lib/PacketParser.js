...
* Constructor
* ---------------------------------------------------------------- */
const PacketParser = function (mDeviceDescription, mManufacturerTable) {
...
* ---------------------------------------------------------------- */
PacketParser.prototype.parse = function (buf, release) {
	let buf_list = [];
	let data = {};
...
PacketParser.prototype._parseOpcProps = function (buf, desc, parse_target_eoj) {
	let buf_list = [];
	let data = {
		opc: {},
		properties: [],
...
PacketParser.prototype._createParseResponse = function (data, buf) {
...
PacketParser.prototype._createParseError = function (code, err, message, buf, data_data) {
	let hex = buf.toString('hex').toUpperCase();
	return {
		result: code,
...
*       - value       | Number | multipleOf 反映済みの EDT が表す数値 (coefficient は考慮していない)
*
*   - state の場合
...
* - 解析に失敗したら null を返す。
* ---------------------------------------------------------------- */
PacketParser.prototype.parsePropertyValue = function (pdata, edt_buf, epc_hex, eoj_hex, release) {
	...
	if (oneof && Array.isArray(oneof)) {
		...
		if (data && data['type'] === 'state' && !data['state'] && /^(number|level)$/.test(oneof[0]['type'])) {
			// oneOf のいずれもヒットしなかった場合は、number の max と
			// min から外れた値、または、level の max を超えた値であり、
			// 最終的に state に落ちたにもかかわらず、対応する値がない状態に
			// なったと思われる。
			// oneOf の最初が number または level の場合に限り、強制的に
			// number または level を適用してパースする
			data = this.parsePropertyValue(oneof[0], edt_buf, epc_hex, eoj_hex, true);
		}
		return data;
	}
	...
	if (type === 'number') {
		...
		let debug_mode = (eoj_hex === '028801' && epc_hex === 'E4') ? true : false;
		edt_data = this._parsePropertyValueObject(pdata, edt_buf, debug_mode);
	}
...
PacketParser.prototype._parsePropertyValueNumber = function (pdata, edt_buf) {
	...
	if (v !== null) {
		if (pdata['multipleOf'] && typeof (pdata['multipleOf']) === 'number') {
			let mo = pdata['multipleOf'];
			if (mo < 1) {
	...
	return {
		...
		number: {
			...
			multipleOf: pdata['multipleOf'] || null,
...
PacketParser.prototype._parsePropertyValueState = function (pdata, edt_buf) {
...
PacketParser.prototype._parsePropertyValueNumericValue = function (pdata, edt_buf) {
	let enum_list = pdata['enum'];
	if (!enum_list || !Array.isArray(enum_list)) {
		return null;
	}
...
PacketParser.prototype._parsePropertyValueLevel = function (pdata, edt_buf) {
	let base = pdata['base'];
...
PacketParser.prototype._parsePropertyValueBitmap = function (pdata, edt_buf) {
	let size = pdata['size'];
...
PacketParser.prototype._parsePropertyValueDateTime = function (pdata, edt_buf) {
	let blen = 7;
	if (pdata['size']) {
		blen = pdata['size'];
	}
...
PacketParser.prototype._parsePropertyValueTime = function (pdata, edt_buf) {
	let blen = 3;
	if (pdata['size']) {
		blen = pdata['size'];
	}
...
PacketParser.prototype._parsePropertyValueRaw = function (pdata, edt_buf) {
...
PacketParser.prototype._parsePropertyValueArray = function (pdata, edt_buf) {
...
PacketParser.prototype._determineByteLengthOfPropertyValue = function (pdata, array_len, debug_mode) {
	let oneof = pdata['oneOf'];
	if (oneof) {
...
PacketParser.prototype._parsePropertyValueObject = function (pdata, edt_buf, debug_mode) {
	let props = pdata['properties'];
	...
	for (let i = 0; i < props.length; i++) {
		let p = props[i];
		let name = p['name'];
		let s = this._determineByteLengthOfPropertyValue(p['element'], array_len, debug_mode);
		if (s === null) {
			err = true;
			break;
			...
			offset += s;
			if (/^numberOf/.test(name) && p['element']['type'] === 'number') {
...
PacketParser.prototype._convBufListToHexString = function (buf_list) {
	let hex_list = [];
